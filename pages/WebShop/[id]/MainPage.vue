        ...
        <div class="flex items-center justify-between h-16">
          ...
          <div class="hidden md:block">
            <div class="ml-10 flex items-baseline space-x-4">
              ...
              <NuxtLink :to="basePath + '/cart'" class="text-gray-800 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Panier</NuxtLink>
              <NuxtLink :to="basePath + '/orders'" class="text-gray-800 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Mes commandes</NuxtLink>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <div v-for="product in products" :key="product.id" class="bg-white shadow-md rounded-md overflow-hidden">
          <a @click.prevent="goToProductDetails(product.id, product)" class="cursor-pointer">
            <div class="p-4">
...
const goToProductDetails = (productId: string, product : Products) => {
